			...
			<div class="container">
				<div class="row">
									...
									<div v-for="order in this.cart.orderItemList" :key="order.id">
										...
										<div class="item head clearfix" v-for="p in order.productList" :key="p.id">
											<span class="cart_img" style="float: left;"><img width="50px;" :src="API.BASE_SERVER_URL+p.defaultImg"/></span> <span
												...
												<span>
													<template>
													  <el-input-number v-model="order.quantity"  controls-position="right" @change="handleChange" :min="1" :max="10" size="mini">
														  {{order.quantity}}
													  </el-input-number>
													</template>
												</span>
										...
										<button id="updateCart" @click="updateCart()" type="button" class="btn btn-success margin-top-20 pull-right noradius" v-show="!emptyCart">
											<i class="glyphicon glyphicon-ok"> </i> 更新购物车
										</button>
										<button id="removeCart" @click="removeCartAll" type="button" class="btn btn-danger margin-top-20 pull-right noradius" v-show="!emptyCart">
					...
					<div style="float: right;background: wheat;width: 200px;" v-if="this.cart.price != 0">
						...
						<br />
						<br />
						<button id="updateCart" @click="PayOrder" type="button" class="btn btn-success margin-top-20 pull-right noradius" v-show="!emptyCart">
...
//            window.location.href="payment";
      ...
      updateCart(){
        ...
        this.$post(this.API.API_URL_CART_ITEMS_UPDATE,{
        	userId:userid,
        	type:1,
        	orderItemList: vm.cart.orderItemList
        }).then(res => {
        	  if(res.errorCode == 0) {
